...
int iCount=0,displayCount=0,s=0,s2=0,m=0,m2=0,timbre=0,pulse=0,p=0;
byte vector[]={0x3F, 0x6, 0x5b, 0x4F, 0x66, 0x6D, 0x7D, 0x7, 0x7F, 0x6F};
long ini;
String cadena="",cadena2="";
char aux='\0';
boolean permite=false,inGame=false,startGame=false,win=false;
int iCount2=0,turno=0;
...
void checkKB(int a) {
  ...
  if (digitalRead(42) == 0) {
    if (cadena.length() < 20) {
      if (a == 0) {aux = '1'; timbre=3000;}
      if (a == 1) {aux = '2'; timbre=3100;}
      if (a == 2) {aux = '3'; timbre=3200;}
      pulse = 1;
    }
  }
  if (digitalRead(43) == 0) {
    if (cadena.length() < 20) {
      if (a == 0) {aux = '4'; timbre=3300;}
      if (a == 1) {aux = '5'; timbre=3400;}
      if (a == 2) {aux = '6'; timbre=3500;}
      pulse = 1;
    }
  }
  if (digitalRead(44) == 0) {
    if (cadena.length() < 20) {
      if (a == 0 && !inGame || a == 0&& startGame){aux = '7'; timbre=3600;}
      if (a == 1) {aux = '8'; timbre=3700;}
      if (a == 2) {aux = '9'; timbre=3800;}
      pulse = 1;
    }
  }
  if (digitalRead(45) == 0) {
